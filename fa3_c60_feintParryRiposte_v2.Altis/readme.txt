MISSION README
==============

Mission: Feint! Parry! Riposte!
Version: 0-0-3
Developer(s): Ojas Sarup (SuicideKing)
Description: NATO JTF assaults Altis Internation Airport



README CONTENTS
===============

01. VERSION HISTORY
02. COPYRIGHT STATEMENT
03. TERMS OF USE
04. LEGAL DISCLAIMER
05. INSTALLATION
06. REQUIRED ADDONS
07. NOTES
08. CHANGE HISTORY



01. VERSION HISTORY
===================

Version | Date | Notes

0-0-1 | 01 01 2016 | Holiday special. First release.
0-0-2 | 04 06 2016 | Tweaks and Fixes, V2
0-0-3 | 26 06 2016 | Tweaks, v3
0-0-4 | 06 07 2016 | Tweaks, v4



02. COPYRIGHT STATEMENT
=======================

This mission is (c)2016 Ojas Sarup. All rights reserved.



03. TERMS OF USE
================

This mission (hereafter 'Software') contains files to be used in the PC simulator ArmA 3. To use the Software you must agree to the following conditions of use:

1. Ojas Sarup alias "SuisideKing" (hereafter 'The Author(s)') grant to you a personal, non-exclusive license to use the Software.

2. The commercial exploitation of the Software without written permission from The Author(s) is expressly prohibited.



04. LEGAL DISCLAIMER
====================

The Software is distributed without any warranty; without even the implied warranty of merchantability or fitness for a particular purpose. The Software is not an official addon or tool. Use of the Software (in whole or in part) is entirely at your own risk.



05. INSTALLATION
================

To begin using the Software:

1. Move the file fa3_c60_feintParryRiposte_v1.Altis.pbo into the following directory (assuming you have the Steam version of ArmA 3):

C:\Program Files (x86)\Steam\steamapps\common\Arma 3\MPMissions



06. REQUIRED ADDONS
===================

None, but check out a list of recommended ones here:
http://www.folkarps.com/forum/viewtopic.php?f=43&t=1382


07. NOTES
=========

May want to consider adding CAS/airborne enemy reinforcements. Server performance is not too good.



08. CHANGE HISTORY
==================

Version | Date

0-0-1 | 01 01 2016
First release

0-0-2 | 04 06 2016
v2, Eden-ready
Reduced number of enemy EI spawned
Tweaked some AI modes
Tweaked garrisons
Added some statics
Tweaked weapons of enemy APCs
Added cover map module
Tweaked/removed some markers
Fixed locality issue caused by incorrect implementation of createGarrison
Updated readme

0-0-3 | 26 06 2016
v3
Edited breifing
Cleaned up markers
Tweaked enemy vehicles
Added some props

0-0-4 | 06 07 2016
v4
Added a notification for the shipyard
Slowed QRF slightly
Made objective trigger areas smaller
Some tweaks to markers